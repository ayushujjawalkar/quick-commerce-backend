...
const schemas = {
  ...
  // Create order (UPDATED to allow full snapshot data)
  createOrder: Joi.object({
    shopId: Joi.string().regex(/^[0-9a-fA-F]{24}$/).required(),
    ...
    items: Joi.array().items(Joi.object({
      ...
      // Allow optional variant fields
      variantId: Joi.string().regex(/^[0-9a-fA-F]{24}$/).optional().allow(null, ''),
      variantName: Joi.string().optional().allow(''),
      
      // Allow snapshot fields (Required by Mongoose Schema)
    ...
    // Added Pricing Object Schema
    pricing: Joi.object({
      subtotal: Joi.number().required(),
      discount: Joi.number().default(0),
      tax: Joi.number().required(),
      deliveryFee: Joi.number().default(0),
      platformFee: Joi.number().default(0),
